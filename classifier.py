...
def read_dataset():
    ...
    return df1['Clean Text'].values, df1['Publication Type'].values, df2['Clean Text'].values, df2['Publication Type'].values


def preprocess_data(data1, labels1, data2, labels2):
...
def clean_dataset(X):
    ...
    for idx in range(X.shape[0]):
        X[idx] = ' '.join([word for word in X[idx].split() if word not in cachedStopWords])
    return X
...
if __name__ == "__main__":
    ...
    data1, labels1, data2, labels2 = read_dataset()
    
    data, labels = preprocess_data(data1, labels1, data2, labels2)

    feature_matrix = feature_extraction(data)

    x_train, x_test, y_train, y_test = train_test_split(feature_matrix, labels, test_size=0.40)
